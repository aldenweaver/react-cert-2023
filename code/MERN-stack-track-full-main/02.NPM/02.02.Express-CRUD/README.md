...
These 2 blocks of code are the middleware on this application file. Before a request can be made to `localhost:3000/`, the middleware is preparing a console log of metadata from the request, and preparing a random number between 1 and 10.
...
Next, we're going to set up a response to localhost:3000/ where the client will receive the local data we just set up:
...
Dynamic parameters are a way to query for information by placing text after the URL extention. URL extensions can be used to reach different web pages, but the text beyond that can be captured in order to search a database for specific items. In this lesson, there is local data that exists on the server itself, and we will be using that to test CRUD functionality. 
...
- Run the server by going to the terminal and using the command `node index.js`
- Use Postman to perform a POST request to `localhost:3000/` and in the Body tab (set text to JSON), make sure to fill it with the keys that match the keys of the original data, and fill the values with unique values. Also test a name that conflicts with the data, so you can see what a failure looks like.
- Once it works, cut the server off by pressing `ctrl + c` in the terminal
